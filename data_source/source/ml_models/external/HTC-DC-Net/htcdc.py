...
try:
    ...
    def chamfer_distance(x, y, x_lengths=None, y_lengths=None):
        """Small CPU smoke-test fallback for 1D Chamfer distance."""
        losses = []
        for index in range(x.shape[0]):
            xi = x[index, : x_lengths[index]] if x_lengths is not None else x[index]
            yi = y[index, : y_lengths[index]] if y_lengths is not None else y[index]
            distances = torch.cdist(xi, yi)
            losses.append(distances.min(dim=1).values.mean() + distances.min(dim=0).values.mean())
...
def adapt_first_conv(model, in_channels):
    """Adapt an EfficientNet-style RGB stem convolution to more input channels."""
    if in_channels == 3:
        return
    if not hasattr(model, "conv_stem"):
        raise AttributeError("Cannot adapt input channels: backbone has no conv_stem.")
    old_conv = model.conv_stem
    if old_conv.in_channels == in_channels:
        return
    if old_conv.in_channels != 3:
        raise ValueError(f"Expected a 3-channel stem, found {old_conv.in_channels}.")
    ...
    with torch.no_grad():
        ...
        repeats = int((in_channels + old_conv.in_channels - 1) / old_conv.in_channels)
        repeated_weight = old_conv.weight.repeat(1, repeats, 1, 1)[:, :in_channels, :, :]
        new_conv.weight.copy_(repeated_weight * (old_conv.in_channels / in_channels))
...
class UBins(BaseHeightPredictor):
    def __init__(self, cfgs):
        ...
        in_channels = int(cfgs.get("in_channels", 3))
        if self.backbone_name.startswith("efficientnetb"):
            ...
            adapt_first_conv(basemodel, in_channels)
        ...
        self.height_loss_weighting = cfgs.get("height_loss_weighting", "none")
        self.height_log_alpha = float(cfgs.get("height_log_alpha", 0.5))
        self.height_bin_edges = cfgs.get("height_bin_edges", [10, 25, 50])
        self.height_bin_weights = cfgs.get("height_bin_weights", [1, 2, 4, 8])
        self.background_loss_weight = float(cfgs.get("background_loss_weight", 0.0))
        if self.loss_p:
    ...
    def height_weights(self, target):
        if self.height_loss_weighting in [None, False, "none"]:
            return torch.ones_like(target)
        if self.height_loss_weighting == "log":
            return 1 + self.height_log_alpha * torch.log1p(torch.clamp(target, min=0))
        if self.height_loss_weighting == "bins":
            weights = torch.ones_like(target) * float(self.height_bin_weights[0])
            for edge, weight in zip(self.height_bin_edges, self.height_bin_weights[1:]):
                weights = torch.where(target >= float(edge), torch.ones_like(target) * float(weight), weights)
            return weights
        raise ValueError(f"Unknown height_loss_weighting: {self.height_loss_weighting}")

    def masked_l1(self, prediction, target):
        mask = target > 0
        height_weights = self.height_weights(target)
        weighted_mask = mask.to(torch.float) * height_weights
        denominator = weighted_mask.sum()
        if denominator <= 0:
            return F.l1_loss(prediction, target)
        return (F.l1_loss(prediction, target, reduction='none') * weighted_mask).sum() / denominator

    def background_l1(self, prediction, gt):
        if self.background_loss_weight <= 0:
            return prediction.new_tensor(0.0)
        if "mask" in gt:
            background_mask = gt["mask"] <= 0
        else:
            background_mask = gt["ndsm"] <= 0
        denominator = background_mask.to(torch.float).sum()
        if denominator <= 0:
            return prediction.new_tensor(0.0)
        return (torch.abs(prediction) * background_mask.to(torch.float)).sum() / denominator
    ...
    def get_losses(self, pred, gt, weights = [0.25, 0.5, 1]):
        ...
        for i in range(len(pred["ndsm_intermediate"])):
            losses.update({
                "mae_"+str(i): self.masked_l1(pred["ndsm_intermediate"][i], gt["ndsm"]),
                "background_mae_"+str(i): self.background_l1(pred["ndsm_intermediate"][i], gt),
            ...
            losses.update({
                "loss_total_"+str(i): losses.get("mae_"+str(i), 0) + self.background_loss_weight * losses.get("background_mae_"+str(i), 0) + self.chamfer_w * losses.get("bin_chamfer_"+str(i), 0) + losses.get("cross_entropy_"+str(i), 0) + losses.get("loss_probability_"+str(i), 0) + losses.get("loss_probability_bg_"+str(i), 0)
            })
        loss_total = 0
        for i in range(len(pred["ndsm_intermediate"])):
