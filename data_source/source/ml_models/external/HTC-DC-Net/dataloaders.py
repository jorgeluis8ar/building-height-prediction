...
def image_to_channels_first(image):
    if image.ndim == 2:
        return image[None, :, :]
...
def get_train_val_dataloaders(cfgs):
    ...
    use_vis = cfgs.get('use_vis', False)
...
class GBHDataset(torch.utils.data.Dataset):
    def __init__(self, data_dir, data_mode, image_size=256, use_mask=False, crop=None, hflip=False, \
        normalize=True, is_validation=False, overfit=False, use_normal=False, rcnn=False, **kwargs):
    ...
    def __getitem__(self, index):
        ...
        image = torch.tensor(image_to_channels_first(io.imread(image_path).astype(np.float32)))
