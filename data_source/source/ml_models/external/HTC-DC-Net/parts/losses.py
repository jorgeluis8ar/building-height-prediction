...
def _stable_target_distribution_inputs(probs, bin_edges, gt):
    """Return a finite target-bin assignment, including zero-height pixels."""
    lower = bin_edges[:, :-1, None, None]
    upper = bin_edges[:, 1:, None, None]
    bin_ind = (gt > lower) & (gt <= upper)
    below = gt <= lower[:, :1]
    above = gt > upper[:, -1:]
    bin_ind[:, :1] |= below
    bin_ind[:, -1:] |= above
    mode_prob = (probs * bin_ind).sum(dim=1, keepdim=True).clamp(1e-4, 1 - 1e-4)
    bin_lower_edge = (bin_ind * lower).sum(dim=1, keepdim=True)
    bin_upper_edge = (bin_ind * upper).sum(dim=1, keepdim=True)
    return bin_ind, mode_prob, bin_lower_edge, bin_upper_edge

def gaussian_loss(probs, bin_edges, gt, masked=False, reduction=True):
    with torch.no_grad():
        bin_ind, mode_prob, bin_lower_edge, bin_upper_edge = _stable_target_distribution_inputs(
            probs, bin_edges, gt
        )
        std = (bin_upper_edge - bin_lower_edge) / 2. / math.sqrt(2) / torch.erfinv(mode_prob)
        std = std.abs().clamp_min(1e-6)
...
def uniform_loss(probs, bin_edges, gt, masked=False, reduction=True):
    with torch.no_grad():
        ...
        std = ((bin_upper_edge - bin_lower_edge) / mode_prob).abs().clamp_min(1e-6)
