"""
Search NYC/LA Planet Scenes With View Angle
...
Description:
    Searches Planet PSScene metadata for New York City and Los Angeles only,
    using each city's 5km AOI buffer. The output is separate from the main
    all-city inventory and includes Planet's view_angle item property. This
    script is metadata-only: it never activates, orders, or downloads imagery
    assets.
...
def load_planet_api_key() -> str | None:
    """Load Planet API key from local ignored file or environment."""
    local_credentials = SCRIPT_DIR / "PLANET_API.py"
    if local_credentials.exists():
        spec = importlib.util.spec_from_file_location("planet_api_credentials", local_credentials)
        if spec is None or spec.loader is None:
            raise RuntimeError(f"Could not load Planet credentials from {local_credentials}")
        module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(module)
        api_key = getattr(module, "PL_API_KEY", None)
        if api_key:
            return str(api_key)
...
DEFAULT_TARGET_CITIES = ["los_angeles", "new_york_city"]
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Search downloadable Planet PSScene metadata for NYC and LA using "
            "5km city-specific AOI buffers, saving view_angle. This command "
            "does not activate, order, or download assets."
        )
    )
    parser.add_argument(
        "--aoi-dir",
    ...
    parser.add_argument(
        ...
        / "planet_imagery"
        / "generated"
        / "nyc_la_scenes_results_planet_with_view_angle.csv",
    )
    parser.add_argument(
        "--start-date",
        help=(
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "City slug to query. Defaults to los_angeles and new_york_city. "
            "May be supplied more than once to override the default list."
        ),
    )
    parser.add_argument(
...
def main() -> None:
    ...
    if args.cities:
        ...
        requested = set(DEFAULT_TARGET_CITIES)
    ...
    print(
        f"Metadata-only NYC/LA Planet view-angle search: {len(aoi_files)} cities, "
    ...
    if api_key:
        print("Authentication: local Planet API key or PL_API_KEY environment variable.")
