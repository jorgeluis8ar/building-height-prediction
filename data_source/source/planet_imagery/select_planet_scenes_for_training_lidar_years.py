#!/usr/bin/env python3
"""Select eight PlanetScope scenes per defensible LiDAR acquisition year.
...
Temporal policy:
  * Semicolon-separated exact years are processed independently from 2016 on.
  * The two endpoints of a documented two-year flight/project range are used.
  * Broad campaign, catalogue, and national ranges are not expanded into
    invented annual acquisition dates; they are reported for metadata review.
  * Pre-2016 acquisition years are reported as unavailable in the existing
    Planet metadata period and are never replaced by a later year.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--asset-candidates-per-city-year", type=int, default=60,
        help="Maximum same-year candidates whose Planet asset lists are checked.",
    )
...
async def main() -> None:
    ...
    try:
        ...
        if args.asset_candidates_per_city_year < TARGET_SCENES_PER_YEAR:
            raise ValueError("asset-candidates-per-city-year must be at least 8")
        ...
        missing_metadata = [] if combined_metadata_by_city is not None else [
            ...
            and not (args.metadata_dir / f"{city['city_slug']}_planet_scenes.csv").is_file()
        ]
        if missing_metadata:
        ...
        attempts = completed = skipped = 0
        print(
            f"LiDAR-year selection-only run for {len(selected_cities)} of 94 cities. "
        ...
        for number, (_, city) in enumerate(selected_cities.iterrows(), start=1):
            ...
            interpretation = interpret_lidar_years(city)
            years = interpretation["eligible_years"]
            if not years:
            ...
            for year in years:
                attempts += 1
                stem = f"{slug}_{year}"
                city_output = args.output_dir / "by_city_year" / f"{stem}_selected_planet_scenes.csv"
                summary_output = args.output_dir / "by_city_year_summary" / f"{stem}_selection_summary.csv"
                if city_output.exists() and summary_output.exists() and not args.overwrite:
                    print(f"[{number}/{len(selected_cities)}] {stem}: skipped completed output", flush=True)
                    skipped += 1
                    continue
                print(f"[{number}/{len(selected_cities)}] {stem}: selecting up to 8 scenes", flush=True)
                same_year_metadata = metadata[metadata["selection_calendar_year"] == year].copy()
                candidates = all_candidates[all_candidates["selection_calendar_year"] == year].copy()
                shortlist = global_selector.metadata_shortlist(
                    candidates, args.asset_candidates_per_city_year
                )
                ...
                selected = select_same_year_scenes(compatible, year, interpretation["rule"])
                for field in [
                    "lidar_acquisition_years", "lidar_acquisition_start_date",
                    "lidar_acquisition_end_date", "lidar_acquisition_date_precision",
                    "lidar_acquisition_date_source",
                ...
                output_columns = [
                    ...
                    column for column in global_selector.SELECTION_COLUMNS + ADDED_SELECTION_COLUMNS
                    if column not in metadata.columns
                ]
                global_selector.atomic_write_csv(selected, city_output, output_columns)
                summary = summary_row(
                    city, year, interpretation, len(metadata), len(same_year_metadata),
                    len(candidates), len(compatible), selected,
                )
                global_selector.atomic_write_csv(
                    pd.DataFrame([summary]), summary_output, SUMMARY_COLUMNS
                )
                completed += 1
                print(
                    f"  selected={len(selected)} shortfalls={summary['shortfall_reason'] or 'none'}",
                    flush=True,
                )

        rebuild_outputs(args.output_dir)
        log_lines[0] = "status=SUCCESS"
        log_lines.extend([
            f"selected_city_batch_count={len(selected_cities)}",
            f"city_year_attempt_count={attempts}",
        ...
        log_path.write_text("\n".join(log_lines) + "\n", encoding="utf-8")
        print(f"SUCCESS: LiDAR-year selection completed. Log: {log_path}", flush=True)
...
if __name__ == "__main__":
    try:
        ...
        print("Interrupted: completed city-year outputs remain resumable.", file=sys.stderr)
        sys.exit(130)
